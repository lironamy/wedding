...
export async function POST(request: Request) { // Changed to POST as it triggers an action
    try {
        ...
        // Find contacts for whom invitationSent is false
        const unsentContacts = await Contact.find({ invitationSent: false });
        ...
        for (const contact of unsentContacts) {
            ...
            const messageBody = `Hello ${contact.name},\n\nYou're invited to Sarah & John's wedding! Help us create a fun photo experience by uploading your selfie: ${selfieUploadUrl}\n\nWe can't wait to celebrate with you!`;
